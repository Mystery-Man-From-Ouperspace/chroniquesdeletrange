...
export class EntitySheetHelper {
      ...
      if ( el.classList.contains("attribute-key") ) {
        ...
        for ( let i = 0; i < groups.length; i++ ) {
          if (groups[i].value === val) {
            ui.notifications.error(game.i18n.localize("CDE.NotifyAttrDuplicate") + ` (${val})`);
            el.value = oldVal;
            attrError = true;
            break;
          }
  ...
  static onAttributeRoll(event) {
    ...
    const shorthand = game.settings.get("chroniquesdeletrange", "macroShorthand");
  ...
  static validateGroup(groupName, document) {
    ...
    if ( groups.includes(groupName) ) {
      ui.notifications.error(game.i18n.localize("CDE.NotifyGroupDuplicate") + ` (${groupName})`);
      return false;
    }

    // Check for group keys that match attribute keys.
    if ( attributes.includes(groupName) ) {
      ui.notifications.error(game.i18n.localize("CDE.NotifyGroupAttrDuplicate") + ` (${groupName})`);
      return false;
    }

    // Check for reserved group names.
    if ( ["attr", "attributes"].includes(groupName) ) {
      ui.notifications.error(game.i18n.format("CDE.NotifyGroupReserved", {key: groupName}));
      return false;
    }

    // Check for whitespace or periods.
    if ( groupName.match(/[\s|\.]/i) ) {
      ui.notifications.error(game.i18n.localize("CDE.NotifyGroupAlphanumeric"));
      return false;
    }
    return true;
  }
  ...
  static async deleteAttributeGroup(event, app) {
    ...
    new Dialog({
      title: game.i18n.localize("CDE.DeleteGroup"),
      content: `${game.i18n.localize("CDE.DeleteGroupContent")} <strong>${group.val()}</strong>`,
      buttons: {
        confirm: {
  ...
  static async createDialog(data={}, options={}) {
    ...
    const templates = collection.filter(a => a.getFlag("chroniquesdeletrange", "isTemplate"));
    const defaultType = this.metadata.types[0];
    const types = {
      [defaultType]: game.i18n.localize("CDE.NoTemplate")
    }
    ...
    return Dialog.prompt({
      ...
      callback: html => {
        ...
        if ( template ) {
          ...
          delete createData.flags.chroniquesdeletrange.isTemplate;
        }
